...
const FluxCanvas: React.FC<FluxCanvasProps> = ({ theme, renderMode }) => {
  ...
  useEffect(() => {
    ...
    // Default dimensions to prevent 0 division/modulo errors
    let width = Math.max(window.innerWidth, 100);
    let height = Math.max(window.innerHeight, 100);
    let tick = 0;
    ...
    const drawCelestial = (c: CelestialBody, screenX: number, screenY: number) => {
        if (!isFinite(screenX) || !isFinite(screenY)) return; // Safety check
    ...
    const render = () => {
      // Loop request at start to ensure it continues even if we return early
